...
type Nested struct {
	TestObject struct {
		SliceIn  []int    `validate:"in:10,15,20"`
...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				v: struct {
					SliceInt    []int    `validate:"in:-5,-10"`
					SliceString []string `validate:"len:5"`
				}{
					SliceInt:    []int{-10, -5, -10},
					SliceString: []string{"12375", "23456", "34567"},
				},
			},
			wantErr: false,
		},
			...
			args: args{
				v: Nested{
					TestObject: struct {
						...
						SliceIn:  []int{10, 20, 15},
						SliceLen: []string{"ab", "cd", "ec"},
						MaxInt:   9,
					},
					foo: 100,
				},
		...
		{
			name: "wrong type inside the structure",
			args: args{
				v: struct {
					BookMap map[string]int `validate:"in:10,11,12"`
				}{
					BookMap: map[string]int{"A": 10, "B": 11, "C": 12},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
